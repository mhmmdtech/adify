...
@push('head-tags')
    <title>افزودن شرکت جدید به لیست سیاه</title>
...
@push('script-tags')
    ...
    <script defer src="{{ asset('admin-assets/js/pages/createCompanyBlacklist.js') }}"></script>
...
@section('content')
    ...
    <section class="w-100 mg-top-1">
        <form id="create-company-blacklist-form"
            class="w-100 d-flex flex-wrap flex-row justify-content-between mg-top-1 mg-bottom-1 job-form" novalidate
            method="post" enctype="multipart/form-data" action="{{ route('admin.companies.blacklist.store') }}"
            data-secondary-action="{{ route('api.admin.companies.blacklist.store') }}">
            @csrf
