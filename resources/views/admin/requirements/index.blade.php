...
@section('content')
    ...
    <section class="w-100 mg-top-1 overflow-x job-ads">
        <table class="table-layout-fixed w-100 border-collapse color-primary ads-table" id="requirements__table">
            <thead>
            ...
            <tbody>
                @foreach ($requirements as $requirement)
                    <tr>
                        ...
                        <td class="text-center border-color-primary pd-half ads-table__cell">
                            {{ georgianToJalaliDate($requirement->created_at) }}
                        </td>
                        <td class="text-center border-color-primary pd-half ads-table__cell">
                            <a href="{{ route('admin.requirements.show', ['requirement' => $requirement->id]) }}"
                            ...
                            <form style="display: inline;" method="post"
                                action="{{ route('admin.requirements.destroy', ['requirement' => $requirement->id]) }}">
                                @csrf
                                {{ method_field('delete') }}
                                <button type="submit"
