...
@push('script-tags')
    ...
    <script defer src="{{ asset('admin-assets/js/pages/createRequirement.js') }}"></script>
@endpush
...
@section('content')
    ...
    <section class="w-100 mg-top-1">
        <form id="create-requirement-form"
            class="w-100 d-flex flex-wrap flex-row justify-content-between mg-top-1 mg-bottom-1 job-form" novalidate
            method="post" enctype="multipart/form-data" action="{{ route('admin.requirements.store') }}"
            data-secondary-action="{{ route('api.admin.requirements.store') }}">
            @csrf
